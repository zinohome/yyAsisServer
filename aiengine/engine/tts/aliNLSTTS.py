...
class AliNLSTTS(BaseEngine):
    ...
    class NlsWorker:
        ...
        def on_error(self, message, *args):
            log.error(f"[{self._config.NAME}] On error: {message}, args: {args}")
            self._error_message = str(message)
            self._error_occurred = True
            self._completion_event.set() # Signal completion even on error

        def on_close(self, *args):
            log.debug(f"[{self._config.NAME}] On close: args: {args}")
        ...
        def on_completed(self, message, *args):
            log.debug(f"[{self._config.NAME}] On completed: {message}")
        ...
        def synthesize(self) -> Optional[bytes]:
            ...
            log.debug(f"[{self._config.NAME}] Starting TTS synthesis for text: {self._text[:50]}...")
            ...
            if self._error_occurred:
                log.error(f"[{self._config.NAME}] Synthesis failed: {self._error_message}")
                return None
    ...
    async def run(self, input: TextMessage, **kwargs) -> Optional[AudioMessage]:
        log.info(f"[{self.cfg.NAME}] Received text for TTS: {input.data[:50]}...")
        # 参数校验
        ...
        if not input.data:
            log.warning(f"[{self.cfg.NAME}] Received empty text for TTS.")
            return None
        ...
        if audio_content:
            ...
            log.info(f"[{self.cfg.NAME}] TTS synthesis successful. Audio size: {len(audio_content)} bytes")
            return AudioMessage(
                data=audio_content,
                format=audio_format,
