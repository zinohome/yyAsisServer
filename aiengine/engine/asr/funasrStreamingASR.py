...
from aiengine.engine.builder import ASREngines
from aiengine.protocol import *
from aiengine.engine.engineBase import StreamBaseEngine
...
class FunasrStreamingAsr(StreamBaseEngine):
    ...
    async def _task_recv(self, adhWebsocket: WebSocket, funasrWebsocket: websockets.ClientConnection, mode: str):
        ...
        try:
            ...
            log.debug("funasrWebsocket closed, task_recv exit")
        except Exception as e:
            log.error(f"FunasrStreamingAsr task_recv error: {e}")
            await WebSocketHandler.send_message(adhWebsocket, WS_SEND_ACTION_TYPE.ERROR, str(e))
