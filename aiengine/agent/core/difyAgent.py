...
from aiengine.utils import httpxAsyncClient, log, resonableStreamingParser
...
class DifyApiAgent(BaseAgent):
    async def createConversation(self, **kwargs) -> str:
        ...
        if 'conversation_id' not in data:
            log.error(f"[AGENT] Engine create conversation failed: {data}")
            return ""
                ...
                async def generator(coversaiotnIdRequire):
                    ...
                    async for chunk in response.aiter_lines():
                        ...
                        if "message" in data["event"] and 'answer' in data:
                            log.debug(f"[AGENT] Engine response: {data}")
